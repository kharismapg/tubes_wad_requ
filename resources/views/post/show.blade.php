<x-app-layout>
    <div class="py-10 bg-gray-50 min-h-screen">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Back Button -->
            <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-900 mb-8 transition-colors">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <!-- Left: Poster + Description + Metadata -->
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <!-- Main Content Card -->
                    <div class="bg-white rounded-2xl p-6 lg:p-8 shadow-sm border border-gray-200">
                        <!-- Title & Meta Header -->
                        ...
                        <!-- Description Text -->
                        <div class="mb-8">
                            <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wide mb-3">Deskripsi Event</h3>
                                ...
                                <ul class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    @foreach($post->requirements as $index => $requirement)
                                        <li class="flex items-start gap-3 bg-gray-50 p-3 rounded-xl">
                                            <span class="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-100 text-indigo-600 text-xs font-bold flex items-center justify-center mt-0.5">
                                                {{ $index + 1 }}
                                            </span>
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sticky top-6">
                        ...
                        <div class="flex items-center gap-4 mb-6">
                            ...
                            @else
                                <div class="w-12 h-12 rounded-full bg-indigo-50 flex items-center justify-center text-lg font-bold text-indigo-600 ring-2 ring-gray-100 flex-shrink-0">
                        ...
                        @if($post->user_id === Auth::id() || Auth::user()->isAdmin())
                            <div class="space-y-3">
                                <a href="{{ route('post.edit', $post->id) }}" 
                                    class="w-full flex items-center justify-center px-4 py-3 bg-white border-2 border-indigo-50 text-indigo-600 font-bold rounded-xl hover:bg-indigo-50 hover:border-indigo-100 transition-all">
                                    Edit Post
                                </a>
                                <form action="{{ route('post.destroy', $post->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')" class="w-full">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full flex items-center justify-center px-4 py-3 bg-red-50 text-red-600 font-bold rounded-xl hover:bg-red-100 transition-colors">
                                        Hapus Post
                                    </button>
                                </form>
                            </div>
                        @else
                            <div class="space-y-3">
                                <a href="{{ $post->link_pendaftaran }}" target="_blank" 
                                    class="w-full block bg-gradient-to-r from-indigo-600 to-indigo-700 text-white font-bold rounded-xl px-4 py-4 text-center shadow-lg shadow-indigo-200 hover:shadow-indigo-300 transform hover:-translate-y-0.5 transition-all">
                                    Daftar Sekarang
                                </a>
                                <a href="{{ route('report.create', $post->id) }}" 
                                    class="w-full flex items-center justify-center gap-2 px-4 py-3 text-gray-400 text-sm font-medium hover:text-gray-600 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
