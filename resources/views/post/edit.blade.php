                ...
                <form action="{{ route('post.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <select id="kategori" name="kategori" required
                            ...
                            <option value="Kepanitiaan" {{ old('kategori', $post->kategori) == 'Kepanitiaan' ? 'selected' : '' }}>Kepanitiaan</option>
                            <option value="Organisasi" {{ old('kategori', $post->kategori) == 'Organisasi' ? 'selected' : '' }}>Organisasi</option>
                            <option value="Laboratorium" {{ old('kategori', $post->kategori) == 'Laboratorium' ? 'selected' : '' }}>Laboratorium</option>
                            <option value="Seminar" {{ old('kategori', $post->kategori) == 'Seminar' ? 'selected' : '' }}>Seminar</option>
                            <option value="Lomba" {{ old('kategori', $post->kategori) == 'Lomba' ? 'selected' : '' }}>Lomba</option>
                            <option value="Event Kampus" {{ old('kategori', $post->kategori) == 'Event Kampus' ? 'selected' : '' }}>Event Kampus</option>
                        </select>
                        @error('kategori')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="6" required
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white"
                            placeholder="Describe your event, requirements, benefits, etc.">{{ old('deskripsi', $post->deskripsi) }}</textarea>
                        @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="date" id="deadline" name="deadline" value="{{ old('deadline', $post->deadline->format('Y-m-d')) }}" required
                    ...
                    <div class="mb-6">
                        ...
                        <input type="url" id="link_pendaftaran" name="link_pendaftaran" value="{{ old('link_pendaftaran', $post->link_pendaftaran) }}" required
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white"
                    ...
                    <div class="mb-6">
                        <label for="poster" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                            Event Poster <span class="text-gray-500">(Leave empty to keep current)</span>
                        </label>
                        @if($post->poster_path)
                        <div class="mb-4">
                            <p class="text-sm text-gray-500 mb-2">Current Poster:</p>
                            <img src="{{ asset('storage/' . $post->poster_path) }}" alt="Current poster" class="h-48 rounded-lg shadow-lg">
                        </div>
                        @endif
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600 dark:text-gray-400 justify-center">
                                    <label for="poster" class="relative cursor-pointer bg-white dark:bg-gray-700 rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none">
                                        ...
                                        <input id="poster" name="poster" type="file" class="sr-only" accept="image/*" onchange="handleFiles(this.files)">
                                    </label>
                        ...
                        <div id="imagePreview" class="mt-4 hidden">
                            <p class="text-sm text-indigo-600 font-semibold mb-2">New Poster Preview:</p>
                    ...
                    <div class="flex space-x-4">
                        ...
                        <a href="{{ route('post.my-posts') }}" class="px-6 py-3 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-semibold rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition">
                            Cancel
                        </a>
                    </div>
                </form>
